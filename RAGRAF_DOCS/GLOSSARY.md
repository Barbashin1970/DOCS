...
- [Часть XIX. «Сима», голосовой компас и пилот (2026-06)](#часть-xix-сима-голосовой-компас-и-пилот-2026-06)
- [Часть XX. Финансовый скоринг: паспорт решения и гуманистический решатель (2026-06)](#часть-xx-финансовый-скоринг-паспорт-решения-и-гуманистический-решатель-2026-06)
- [Часть XXI. Строительная аналитика Plan-R: иерархия и понятия (2026-07)](#часть-xxi-строительная-аналитика-plan-r-иерархия-и-понятия-2026-07)
- [Часть XXII. Методы ИИ в терминах Центра и вероятностная оценка результативности (2026-09)](#часть-xxii-методы-ии-в-терминах-центра-и-вероятностная-оценка-результативности-2026-09)
  - [Два слоя искусственного интеллекта](#xxii1-два-слоя-искусственного-интеллекта-и-их-нельзя-путать) — модельный «что происходит» и правиловый «что делать»
  - [Термины ИИ в значениях Центра](#xxii2-термины-ии-в-значениях-центра) — прикладная система ИИ, доверенный ИИ, предъявимость
  - [Вероятностная оценка по шагам](#xxii3-вероятностная-оценка-результативности--по-шагам-без-математики) — правило Лапласа для нематематика, с примерами
  - [Почему Лаплас, а не эмпирический байес](#xxii4-почему-правило-лапласа-а-не-эмпирический-байес) — что даст переход и почему он преждевремен
...
**Edge computing** — обработка данных непосредственно на устройстве или рядом с ним, без отправки в облако. Видеодетектор самостоятельно распознаёт лицо и отправляет уже структурированное событие (а не сырое видео) — это edge inference.
...
**RTSP / ONVIF** — стандарты потокового видео и управления CCTV-камерами. РАГРАФ напрямую с ними не работает (получает уже распознанные события), но в Module Library есть паспорта `rtk_ngu_srvr` и `rtk_ngu_sa` видеовендора — это и есть RTSP-кластеры.

**Видеовендоры (российские)** — поставщики видеоаналитических систем. В РАГРАФ Sensor Library есть подтипы для их детекторов: `vd-face`, `vd-person`, `vd-anpr`, `vd-fall`, `vd-smoke`, `vd-fire`, `vd-weapon` (лица / оружие / дым / падение, 2024 PDF #13) и `vd-vehicle-brand`, `vd-accident`, `vd-stop-in-lane`, `vd-pedestrian` (транспорт / ДТП, 2024 PDF #14).
...
**Geofence** — виртуальная граница на карте. Событие «объект пересёк geofence» — типичный триггер для умного города. В РАГРАФ напрямую не реализован, но регламенты видеовендора (`vd-stop-in-lane`, `vd-dropped-cargo`) подразумевают конфигурацию geofence на стороне детектора.
...
### Сценарий «видеодетектор → регламент»

```mermaid
flowchart LR
  CAM["📷 Камера на<br>перекрёстке"]
  VD["🧠 vd-weapon<br>(видеовендор)<br>edge inference"]
...
В РАГРАФ мы **проектируем сам регламент** `R` и **схему события** (что видеодетектор должен прислать). Само распознавание происходит вне РАГРАФ.
...
**Formula DSL (язык формул)** — *вложенный* язык выражений внутри Flow DSL. Здесь работают **два уровня одного языка**: Flow DSL — **внешний** визуальный язык-граф («что соединить»: `датчик → вход → формула → развилка → выход`), а Formula DSL — **внутренний** текстовый язык («что посчитать»), который живёт в поле `expression` узлов `формула` и `время`. Это безопасное подмножество выражений (арифметика, сравнения, `and/or/not`, тернарка), вычисляемое обходом AST по белому списку **без `eval`**, с трёхзначной логикой Клини K3 (истина/ложь/неизвестно), оконными функциями (`rate`, `mean`, `count_above`) и функциями времени (`hour`, `is_night`, `between`, `day_of_week`). Узлы `порог` и `сравнение` — «сахар», сводимый к выражениям Formula DSL. Кратко: **Flow DSL — структура (макро), Formula DSL — логика в узле (микро)**.
...
РАГРАФ — практическая реализация задачного подхода: каждый цифровой регламент — это задача с явной предметной областью, явным запросом (входным событием), явным критерием решенности и явным контекстом цели. См. [docs/STRATEGY-POSITIONING.md](GOTOMARKET/STRATEGY-POSITIONING.md), [docs/TASK-APPROACH.pdf](TASK-APPROACH.pdf).
...
| **4. Вероятностный** | Нечёткие оценочные утверждения, эмпирические законы, прогнозы | Байесовский вывод по правилу Лапласа, бета-распределение | `forecast.py` — апостериорное бета-распределение доли успешных срабатываний Beta(1 + успехи, 1 + провалы); активируется при ≥ 10 наблюдений |
...
**Практическое значение для пользователя РАГРАФ.** Хранение регламента сразу на всех четырёх уровнях позволяет **прогнозировать поведение регламента до запуска**: при достаточной истории исполнений вероятностная оценка показывает, где у регламента слабые места и что меняется со временем. Это **прямая помощь методологу процессов управления** — он видит проблемы заранее, а не по факту инцидента. См. также [REQ-FORECAST-FEATURE.md](REQ-FORECAST-FEATURE.md).

**Как считается вероятностный уровень.** Применяется **правило следования Лапласа**:
к успехам и провалам добавляется по единице, и оценка равна `(1 + успехи) / (2 + прогоны)`.
Отсюда двадцать успехов из двадцати дают не 100 %, а около 95 %: уверенность растёт
с числом наблюдений, а не объявляется сразу.

> **Разбор метода по шагам — [Часть XXII](#часть-xxii-методы-ии-в-терминах-центра-и-вероятностная-оценка-результативности-2026-09).**
> Там же: почему наивное деление «успехи ÷ прогоны» ломается, откуда берётся интервал
> неопределённости, чем правило Лапласа отличается от эмпирического байеса и что даст
> переход к нему. Написано для читателя без математической подготовки.
...
## Часть XX. Финансовый скоринг: паспорт решения и гуманистический решатель (2026-06)

### Главное за минуту

РАГРАФ — не только «город» и «стройка». Тот же движок регламентов решает задачу **кредитного скоринга**: по заявке выдаётся не «чёрный ящик» с одним баллом, а **паспорт решения** из пяти проверяемых слоёв с пошаговой трассой и объяснением. Демо работает на **двух банках сразу** — РФ (Сбер, регламент `credit-scoring-passport`) и Мексика (Plata, регламент `plata-card-scoring`) — это доказывает переносимость не только между отраслями, но и между юрисдикциями: меняются параметры и нормативные стены, каркас ВИЛНС и движок неизменны. Источник идеи — «Decision Passport 5I-CFFA», привязанный к 4-уровневой модели знаний Д. Е. Пальчунова.

### Ключевые термины

**Паспорт решения (Decision Passport, 5I-CFFA)** — машиночитаемая карточка кредитного решения из пяти согласованных слоёв (Integrity, Compliance, Fairness, Forecast-Risk, Audit). Вместо одной оценки PD клиент и регулятор видят, **какой слой «загорелся» и почему**. В РАГРАФ паспорт материализуется как `ExecutionResult.trace` (`flow_executor`) + запись в `incident_audit_log`. Сводный вердикт: уровень = максимум критичности сработавших слоёв (0 — одобрено, 2 — ручная проверка / отложено, 3 — отказ).

**Пять слоёв ↔ пять добродетелей.** Гуманистический решатель переименовывает технические слои в добродетели и меняет правило агрегации:

- **Вера (Integrity)** — доверие к данным: пройдена ли KYC-верификация, полны ли поля, подтверждён ли доход. Если данных не хватает — решение «**отложено**» (приглашение донести документ), а не «нет». Это операционализация трёхзначной логики Клини (`unknown` ≠ `false`).
- **Истина (Compliance)** — соответствие закону: стоп-факторы **115-ФЗ** (анти-отмывание), **негативный стоп-статус НБКИ** (текущая просрочка 90+/банкротство/взыскание), возраст ≥ 18, ПДН (DTI) ≤ 0.5. Жёсткая нормативная стена (SHACL) — её **нельзя «купить»** хорошим скорингом. Норматив цитируется в паспорте (`source_document`/`source_clause`).
- **Любовь (Fairness)** — анти-дискриминация: контроль disparate impact по когорте. Срабатывание = **передача на ручную проверку (защита заявителя)**, а не отказ. «Щит клиента».
- **Надежда (Forecast-Risk)** — просчёт риска: PD-индекс по поведенческим признакам (utilization, активные кредиты, заявки за 30 дней, **просрочки 30+ за 12 мес.**). Зелёная/жёлтая/красная зона. Отказ по риску возвращает **дорожную карту к «да»** (3 шага: снизить utilization, закрыть кредит, подтвердить доход). Вероятностная оценка — empirical-Bayes (`forecast.py`, осторожная оценка с 90% credible interval).
- **София (Audit)** — мудрый синтез: провенанс (PROV-O), версия скоринг-политики на момент решения, трасса «событие → правило → действие». Венчающий мета-слой, обеспечивающий прозрачность и достоинство человека. Последнее слово — за человеком (андеррайтер), ИИ — модератор, не автономный агент.

**Гуманистический решатель** — четвёртый способ агрегации после Декарта, Байеса-DBUE и d0sl. В отличие от компенсаторной суммы `U = Σ wₖxₖ` (где Fairness можно «докупить» хорошим PD), добродетели **не взаимозаменяемы**: Истина и Любовь — жёсткие стены (лексикографический приоритет), Надежда — управляемое компенсируемое ядро с дорожной картой, Вера обрабатывает неполноту по Клини, София синтезирует с приоритетом достоинства человека. Каркас канонический: Вера, Надежда, Любовь — три богословские добродетели (1 Кор. 13:13), София — их мать; Истина замыкает. Линия русской софиологии (Соловьёв, Булгаков, Флоренский).

**Нормативная база скоринга** — почему объяснимость обязательна: **EC AI Act** (кредитный скоринг = high-risk AI → управление рисками, логирование, прозрачность, человеческий надзор); **GDPR ст. 22** (ограничение «исключительно автоматических» решений, право на «значимую информацию о логике»); **CFPB/ECOA** (adverse-action notice — конкретные причины отказа); **EBA GL/2020/06**; в РФ — **115-ФЗ**, **152-ФЗ** (персональные данные), **Положение Банка России 590-П**. Паспорт решения — ровно тот «комплект объяснимости», который ждёт регулятор.

**Универсальность паспорта (финансы ↔ медицина).** Паспорт решения — переносимая абстракция: те же пять слоёв описывают и медицинский скрининг ([REQ-MED-FLOW-INTEGRATION.md](REQ-MED-FLOW-INTEGRATION.md)) — Вера/Истина = гейт допуска и деперсонализации данных (152-ФЗ, MRIQC-качество снимка), Любовь = когортная справедливость и «врач — последнее слово», Надежда = маршрутизация по критичности (verdict + confidence, Клини `none` при низкой уверенности), София = PROV-O трасса к нормативу. Это подтверждает тезис Пальчунова: 4-уровневая лестница знаний — **универсальный разъём** для медицинских, городских и финансовых доменов.

### Формальная терминология гуманистического решателя (ВИЛНС)

Каждый термин имеет два прочтения — **финансовое** (рабочее) и **гуманистическое** (ВИЛНС). Полная формализация — в НИР, §5.14 (`OTCHET-NIR-RAGRAF-2026.md`).

**ВИЛНС** — аббревиатура пяти добродетелей паспорта: **В**ера · **И**стина · **Л**юбовь · **Н**адежда · **С**офия. Порядок повторяет порядок слоёв ICFFA (Integrity-Compliance-Fairness-Forecast-Audit). Каркас канонический: Вера, Надежда, Любовь — три богословские добродетели (1 Кор. 13:13), София — их мать, Истина замыкает.

**Гуманистический решатель** — способ свёртки пяти слоёв в решение, в котором добродетели **не взаимозаменяемы**. Четвёртый решатель в ряду после квадратов Декарта, Байеса-DBUE и d0sl. *(Фин.)* способ принять кредитное решение, не сводя заявителя к одному баллу. *(ВИЛНС)* решение проходит через пять добродетелей, а не через весовую сумму.

**Компенсаторная vs не-компенсаторная агрегация.** Компенсаторная (`U = Σ wₖxₖ`, как в классическом скоринге) — любой слой возмещается любым: высокий PD «перевешивает» нарушение честности. Не-компенсаторная (лексикографическая, ВИЛНС) — слои разбиты на ярусы приоритета, жёсткие добродетели не возмещаются. *(Фин.)* отличный риск-балл не отменяет стоп-фактора по закону. *(ВИЛНС)* нельзя «купить» Истину или Любовь хорошей Надеждой.

**Жёсткие стены (Истина, Любовь).** Булевы предикаты яруса 1: нарушение → отказ/обязательная ручная проверка безусловно. *(Фин.)* стоп-факторы 115-ФЗ, ПДН > 0.5, возраст < 18 (Истина); срабатывание анти-дискриминационного контроля (Любовь). *(ВИЛНС)* закон и забота о ближнем — несдвигаемые границы.

**Управляемый риск и дорожная карта (Надежда).** Ярус 3 — единственный компенсируемый/градуированный слой: зелёная / жёлтая / красная зона PD. Отказ по риску возвращает **дорожную карту** — конечный набор шагов к «да». *(Фин.)* снизить utilization < 40 %, закрыть кредит, подтвердить доход; повторная заявка через 30 дней. *(ВИЛНС)* «нет» — это «пока нет, вот путь».

**«Отложено» (Вера, трёхзначность Клини).** Ярус 2: при неполноте данных Integrity = `unknown (u)`, и решение — **отложено** (приглашение донести документ), а не отказ. *(Фин.)* нет подтверждения дохода → «загрузите справку, пересчитаем». *(ВИЛНС)* Вера принимает неизвестность как приглашение, а не приговор (`u ≠ false`).

**Синтез и провенанс (София).** Венчающий мета-слой: сборка паспорта, версия политики на момент решения, аудит-след, объяснимость; последнее слово — за человеком (андеррайтер/врач), ИИ — модератор. *(Фин.)* кто/когда/по какой версии политики принял решение. *(ВИЛНС)* мудрость, объединяющая четыре уровня в целое.

**Инвариант достоинства человека** — принцип, который кодирует решатель: человек не сводится к одному числу; жёсткие добродетели не продаются; «нет» — это всегда «пока нет, вот путь». Поднимает «паспорт превращает отказ в управляемый шаг» с уровня UX до архитектурного принципа.

**Причина-слой и уровень критичности.** Сводный `level = max` приоритета сработавших слоёв (0 — одобрено, 2 — проверка/отложено, 3 — отказ); **причина-слой** = слой с максимальным приоритетом (что «загорелось»). Это семантика `flow_executor`, поэтому решатель реализуем на готовом ядре без изменений.

### Два банка: Россия (Сбер) и Мексика (Plata)

Демо переносимости работает на **двух юрисдикциях** одновременно: меняются параметры заявки и нормативные стены, а каркас ВИЛНС и движок остаются. Регламенты — `credit-scoring-passport` (Сбер, РФ) и `plata-card-scoring` (Plata — необанк в Мексике; продукт — кредитная карта).

**Что отличается между банками** (одна добродетель — разный норматив):

| Слой / признак | Россия (Сбер) | Мексика (Plata) |
|---|---|---|
| Вера · KYC личности | паспорт / ЕСИА (`kycVerified`) | INE / CURP |
| Вера · подтверждение дохода | 2-НДФЛ / выписка (`incomeDocProvided`) | CFDI / estado de cuenta (`incomeVerified`) |
| Истина · долговая нагрузка | **DTI / ПДН** ≤ 0.5, все платежи / доход (`dti`) | **PTI** ≤ 0.4, платёж по карте / доход (`pti`) |
| Истина · анти-отмывание | **115-ФЗ** (`sanctionsHit`) | **PLD / CNBV** (`pldHit`) |
| Истина · стоп-статус бюро | **НБКИ** (`bureauNegativeStatus`) | **Buró de Crédito** (`buroNegativeStatus`) |
| Надежда · спец-драйвер PD | — | запрошенный лимит к доходу (`requestedLimitToIncome`) |

**PD-индекс и зоны** (числовой слой Надежды; веса откалиброваны под продукт банка):
- **Сбер:** `PD = 0.06·dti + 0.05·utilization + 0.02·openLoans + 0.015·recentInquiries + 0.045·delinquencies12m`; зоны **< 0.07** (одобрить) / **0.07–0.20** (проверка) / **> 0.20** (отказ).
- **Plata:** `PD = 0.06·utilization + 0.015·requestedLimitToIncome + 0.03·pti + 0.012·openLoans + 0.008·recentInquiries + 0.045·delinquencies12m`; зоны **< 0.08** / **0.08–0.22** / **> 0.22**. Центр тяжести — utilization и запрошенный лимит к доходу.

**Нормативная база Мексики** (почему «комплект объяснимости» нужен и там): **CNBV** (Comisión Nacional Bancaria y de Valores — банковский регулятор; под ним PLD/anti-lavado); **CONDUSEF** (защита прав потребителя финуслуг → мотивированный отказ + дорожная карта); **Banxico** (ЦБ Мексики, правила по картам); **Buró de Crédito** (кредитное бюро); **Ley Fintech (2018)** (закон о финтех-институтах — рамка необанка Plata). Аналогии 1:1 с РФ: PLD ↔ 115-ФЗ, CONDUSEF ↔ защита прав потребителей, Buró ↔ НБКИ, INE/CURP ↔ паспорт/ЕСИА.

> Те же сведения встроены в интерфейс как **мини-глоссарий на экране домена** «Финансы» (`/domains/finance_scoring`, компонент `FinanceScoringGlossary`) — справка под рукой во время демо.

### Где это в коде РАГРАФ

Фикстуры `backend/data/fixtures/credit-scoring-passport.*` (Сбер) и `plata-card-scoring.*` (Plata); домен `finance_scoring` в `fixtures.py`; сопоставление банк→регламент — `scoring.BANK_REGULATIONS`; генератор обезличенных заёмщиков — `borrower_generator.py`; приём и прогон — `api/scoring.py` (`POST /api/scoring/check-random?bank=`), исполнение — `flow_executor` (5 слоёв-формул + approve-гейт); прогноз — `forecast.py`; демо-прогоны — `backend/scripts/demo_scoring_passport.py` и `demo_plata_card_scoring.py`. UI — вкладка «💳 Скоринг заявок» Ситуационного центра (два `BankJournal`) + мини-глоссарий `FinanceScoringGlossary` на экране домена. Проектная проработка — [REQ-SCORING-ICFFA.md](REQ-SCORING-ICFFA.md), [REQ-SCORING-PLATA.md](REQ-SCORING-PLATA.md).

---

## Часть XXI. Строительная аналитика Plan-R: иерархия и понятия (2026-07)

### Главное за минуту

**Plan-R** — российский строительный PMS (система управления проектами, замена
Primavera P6 / MS Project). РАГРАФ надстраивается над ним: тянет графики/этапы/
просрочки по API, применяет регламенты и доставляет вердикты ответственным. Ниже —
словарь иерархии и аналитических понятий Plan-R, на который опираются регламенты
стройки, страница «Стройка · Plan-R», «Контуры интеграции» и диалоговый аналитик Студии.

### Иерархия EPS (Enterprise Project Structure — «структура проектов предприятия»)

**EPS** = *Enterprise Project Structure* → рус. «структура проектов предприятия» —
иерархическое дерево, по которому Plan-R (и РАГРАФ вслед за ним) адресует любые
данные. Дерево: `space → facility → project → schedule → version → wbs`. В API —
эндпоинт `/eps` (тип узла в поле `type`); у каждого узла собственный `id`.

- **Пространство (space, tenant)** — верхний контейнер организации; в API его `id`
  = `x-tenant-id`. Пример: «ООО Айбим-ПрО» (боевой), «ООО Группа МЕТА» (демо).
  В РАГРАФ пространство выбирается на /planr и в «Контурах интеграции».
- **Группа проектов (facility — англ. «объект/сооружение, группа»)** —
  узел-группировка объектов внутри пространства («Западный филиал», холдинг).
  `parentId` → space.
- **Проект / стройобъект (project)** — конкретный объект строительства (ЖК, БЦ,
  школа, мост). `parentId` → facility. Несёт атрибуты договора (см. ниже).
- **График (schedule)** — план-график работ по объекту. `parentId` → project.
- **Версия (version)** — срез графика: **actual** (рабочая/фактическая) или
  **target** (базовый план/baseline). `parentId` → schedule. Пара actual↔target даёт
  план-факт.
- **WBS / этап (work / task)** — **WBS** = *Work Breakdown Structure* → рус.
  «структура декомпозиции работ»; элемент нижнего уровня — отдельный этап/работа.
  `criticalPath` помечает критический путь. Читается `/wbs?EpsId=<version>`.

### Версии, базовый план и план-факт (EVM-lite)

- **Базовый план (baseline, target-версия)** — зафиксированный план сроков/стоимости,
  эталон для сравнения. РАГРАФ пока читает только actual (target — в роадмапе, P1-B).
- **Факт (actual-версия)** — текущее состояние: прогноз финиша, % выполнения.
- **План-факт (schedule variance)** — отклонение факта от базового плана.
- **EVM (Earned Value Management)** — метод освоенного объёма: **PV** (плановый
  объём), **EV** (освоенный), **CV** (отклонение по стоимости = EV−факт),
  **SV** (по срокам), **SPI**=EV/PV (индекс сроков), **CPI**=EV/факт (индекс
  стоимости), **BAC** (бюджет по завершении).

### Показатели этапа и портфельный риск

- **Просрочка этапа (overdue_days)** — на сколько дней этап отстаёт; в Plan-R
  выводится из `slip_days` (отклонение сроков, «−» = отставание), в РАГРАФ —
  `max(0, −slip)`. Критическая просрочка — ≥14 дней.
- **Критический путь (critical path)** — цепочка этапов, срыв которых сдвигает дату
  сдачи объекта. Флаг `criticalPath` на WBS.
- **% выполнения (progress)** — доля готовности этапа.
- **Стоимость план/факт (cost_plan / cost_fact)** — плановая/фактическая стоимость
  этапа; сумма плановой по объекту — прокси «суммы договора» (`contract_value_million_rub`).
- **Портфель** — совокупность объектов пространства. **Портфельный риск** — агрегатная
  оценка (макс. просрочка, число критических этапов, взвешивание по стоимости).
- **Штраф подрядчику** — санкция за просрочку этапа (по договору). **Эскалация** —
  передача решения на уровень выше (ГИП → руководитель → инвестор/совет директоров).
- **Веха (milestone)** — ключевая контрольная точка проекта (сдача очереди, ввод).

### Атрибуты проекта (EPS-attributes)

Плоские свойства объекта (41 в боевом Plan-R; РАГРАФ читает ~8). Договор:
**генподрядчик**, **заказчик**, **генпроектировщик**, **регистрационный № договора**,
**дата заключения договора**, **регион**, **код проекта**, **статус проекта**.
Регуляторные вехи (булевы): **разрешение на строительство**, **разрешение на ввод**,
**ГлавГосЭкспертиза (ГГЭ)**. Ответственные: **Куратор**, **Координатор**,
**Project Manager / ГИП**, **ФИО руководителя**, **курирующий департамент**.
Прочее: **климатическая зона**, **направление**.

### Интеграция РАГРАФ ↔ Plan-R

- **Датчик `pms-stage-overdue`** — снапшот просроченных этапов (overdue_days,
  stage_label, contractor_id). **`pms-contract-meta`** — реквизиты договора + стоимость.
  **`pms-portfolio`** — сводный кеш портфеля пространства (region=space_id).
- **Контур интеграции (binding)** — привязка `{пространство → регламент → канал
  доставки}`; управляется на экране «Контуры интеграции» и в карточке события потока.
- **Канал доставки** — куда уходит вердикт: Telegram / MAX / LEYKA-задача.
- **Авто-уведомление** — при появлении новых критических этапов РАГРАФ сам шлёт
  дайджест по каналам bindings.
- **Диалоговый аналитик (Студия)** — режим «Проекты Plan-R»: вопросы к портфелю на
  естественном языке; числа считает код (из кеша), LLM формулирует.

### Где это в коде РАГРАФ

Клиент — `services/etl/planr_client.py` (иерархия EPS, WBS_ATTR, fetch_aggregates);
пуллер + авто-уведомление — `services/etl/puller.py` (`pull_planr`, `_autonotify_space`);
кеш/эндпоинты — `api/live.py` (`/api/live/planr/{spaces,portfolio,notify}`); привязки —
`services/planr_bindings_store.py` + `api/planr_bindings.py`; аналитик — `services/sandbox.py`
(`planr_ask`); триплеты — `data/knowledge_seed/planr-construction-analytics.csv`. UI —
`components/planr/*`, «Контуры интеграции» (`/planr-bindings`), режим «Проекты Plan-R» в
Студии. Проектная проработка — [REQ-PLANR-INTEGRATION-EXPANSION.md](PLANR/REQ-PLANR-INTEGRATION-EXPANSION.md),
[REQ-PLANR-CONFIG-BINDINGS.md](PLANR/REQ-PLANR-CONFIG-BINDINGS.md),
[REQ-ANALYST-STUDIO-PLANR-AGENT.md](PLANR/REQ-ANALYST-STUDIO-PLANR-AGENT.md),
[HOWTO-PLANR-API-TOKEN.md](PLANR/HOWTO-PLANR-API-TOKEN.md).
...
## Часть XXII. Методы ИИ в терминах Центра и вероятностная оценка результативности (2026-09)

### Главное за минуту

Часть отвечает на два вопроса, которые задают чаще прочих. Первый: **какие именно методы
искусственного интеллекта применены** — и ответ даётся в значениях, принятых в Центре
искусственного интеллекта НГУ, чтобы утверждение можно было сверить, а не обсудить. Второй:
**как считается вероятностная оценка результативности регламента** — разобрано по шагам,
без математической подготовки.

Опорные документы: пояснительная записка Платформы АРХИ, п. 3.1б (`ARCHI-NSU/docs/gost-19/`);
разбор по модулям — `CII-TIPY-II-MODULI-2026-09.md`; код оценки — `backend/app/services/forecast.py`.

---

### XXII.1. Два слоя искусственного интеллекта, и их нельзя путать

**Слой А — модельный, распознающий.** Превращает физический мир в число или метку: свёрточная
модель по изображению, нейросеть по сигналу, регрессия по телеметрии. Отвечает на вопрос
**«что происходит»**. Обучается на данных, ошибается вероятностно, объясняется плохо.

**Слой Б — правиловый, решающий.** Превращает число в вердикт с адресатом, сроком
и последствием: порог, уровень, предписанное действие. Отвечает на вопрос **«что делать»**.
Не обучается, ошибается только вместе с нормой, объясняется построчно.

**Зачем различение.** У слоёв разная проверяемость (модель предъявляют метриками, правило —
документом и расчётом), разный владелец (модель у разработчика модуля, норму утверждает
предметная служба) и разная роль в требованиях доверия: **достоверность держится на слое А,
а интерпретируемость и предъявимость вывода — на слое Б**.

**Слой Б есть не у всякого модуля, и это нормально.** Инженерный инструмент вправе довести
дело до величины и остановиться. Стык двух слоёв и есть место фреймворка.

---

### XXII.2. Термины ИИ в значениях Центра

**Прикладная система ИИ** — компьютерная система для усиления интеллектуальных возможностей
человека, обладающая возможностями: анализировать большие объёмы данных в ограниченном
интервале времени; вырабатывать и объяснять человеку предлагаемые варианты решения; работать
в интерактивном (диалоговом) режиме; работать в автономном режиме.

> *Оговорка, которую стоит делать честно:* определение говорит о **вариантах** решения
> во множественном числе, а регламент предлагает **одно предписанное действие**. Это
> не упрощение, а свойство предмета: норма не предлагает выбора, она предписывает.

**Классификация алгоритмов ИИ** — относит к алгоритмам ИИ извлечение знаний из данных
и их интерпретацию, распознавание образов, прогнозирование, обучение и самоорганизацию,
а также задачи, решаемые методами интеллектуального анализа данных, логики и нечёткого
вывода, поддержки принятия решений и иными.

**Система доверенного (надёжного) ИИ** — прикладная система ИИ, обеспечивающая выполнение
возложенных задач с учётом дополнительных требований, обеспечивающих доверие к результатам:
**достоверность, безопасность, приватность**.

**Система, основанная на знаниях (rule-based)** — система, в которой **знание отделено
от механизма вывода**. Признак проверяется, а не заявляется: правила лежат отдельными
файлами, а в исполнителе нет ни одной ветви под конкретное правило. Старший по возрасту
раздел искусственного интеллекта, чем машинное обучение.

**Система поддержки принятия решений (СППР)** — вырабатывает вердикт с объяснением
и предписанным действием, но **решения не принимает** и в чужие системы управления
не пишет. Основной класс РАГРАФ.

**Продукционный вывод** — вывод по правилам вида «величина → диапазон → уровень → действие →
адресат → срок». Детерминированный: одни и те же входы дают один и тот же результат всегда.

**Лёгкая онтология** — контролируемый словарь величин предметной области с единицами
измерения и отображением на модели данных источников. Отличается от полноценной онтологии
тем, что **логического вывода на ней не выполняется**.

**Троичная логика Клини, исход «нет данных»** — третий исход правила наряду с «норма»
и «нарушение». При отсутствующей либо просроченной величине вердикт **не выносится**.
Противоположность вероятностной достройке пропусков.

> **Почему это существенно, а не педантично.** Благополучный вердикт по неизмеренной
> величине **неотличим от исправной работы**: он выглядит как «нарушений нет». Это самая
> дорогая ошибка систем такого рода, потому что обнаруживается она только после происшествия.

**Предъявимость** — свойство вердикта быть доказательством, а не мнением. Вердикт нейросети
может быть **верным и при этом не быть доказательством**. Детерминизм нужен там, где вердикт
кладут перед надзором, фондом, страховщиком или судом.

---

### XXII.3. Вероятностная оценка результативности — по шагам, без математики

#### Что вообще измеряется

Каждый прогон регламента заканчивается одним из двух исходов: **критерий решённости выполнен
или не выполнен**. Успех или провал, третьего нет. Нас интересует **доля успеха** — какая
часть прогонов заканчивается выполнением критерия.

На языке математики это **схема Бернулли**: последовательность испытаний с двумя исходами
и неизвестной долей успеха. Неизвестную долю принято обозначать *p*, и задача — оценить *p*
по накопленным прогонам.

#### Наивный способ и почему он ломается

Первое, что приходит в голову: разделить успехи на прогоны.

| Прогонов | Успехов | Наивная оценка | Что с ней не так |
|---:|---:|---:|---|
| 0 | 0 | **не определена** | Делить на ноль нельзя, а показать что-то надо |
| 1 | 1 | **100 %** | Один удачный прогон объявляется гарантией |
| 3 | 3 | **100 %** | Три прогона — не основание обещать безотказность |
| 3 | 0 | **0 %** | И не основание объявить регламент неработающим |
| 200 | 180 | 90 % | А вот здесь оценка уже осмысленна |

Беда наивного способа в том, что он **не различает уверенность**: «три из трёх» и «двести
из двухсот» дают одинаковые 100 %, хотя доверия к ним совершенно разное.

#### Правило следования Лапласа: добавить по одному воображаемому исходу

Приём простой до неприличия: **к успехам прибавить единицу и к провалам прибавить единицу**,
как если бы до начала наблюдений мы уже видели один успех и один провал.

```
оценка = (успехи + 1) / (прогоны + 2)
```

Что из этого получается:

| Прогонов | Успехов | Наивно | **По Лапласу** | Как читать |
|---:|---:|---:|---:|---|
| 0 | 0 | — | **50 %** | Ничего не знаем — честная середина |
| 1 | 1 | 100 % | **67 %** | Один успех — ещё не закономерность |
| 3 | 3 | 100 % | **80 %** | Обнадёживает, но не гарантия |
| 3 | 0 | 0 % | **20 %** | Плохо, но приговор выносить рано |
| 10 | 9 | 90 % | **83 %** | Оценка подтянулась к середине |
| 20 | 20 | 100 % | **95 %** | Уверенность **выросла с числом наблюдений** |
| 200 | 180 | 90 % | **89,6 %** | На больших числах разница исчезает |

**Главное свойство правила:** оценка тем ближе к наивной, чем больше прогонов. На двух
наблюдениях она осторожна, на двухстах — почти совпадает с долей. Уверенность **зарабатывается
наблюдениями, а не объявляется сразу**.

Название историческое: Пьер-Симон Лаплас в XVIII веке решал этим приёмом задачу «солнце
всходило N дней подряд — какова вероятность, что взойдёт завтра». Отсюда «правило следования».

#### Откуда берётся интервал «от и до»

Одного числа мало: «80 %» при трёх прогонах и «80 %» при трёхстах — разные утверждения.
Поэтому вместе с оценкой показывается **интервал**, в котором доля лежит с вероятностью 90 %:
от 5-го до 95-го процентиля.

Технически это **бета-распределение** — если совсем просто, кривая, показывающая, насколько
правдоподобно каждое значение доли. При трёх прогонах кривая широкая и низкая: правдоподобно
почти всё. При двухстах — узкая и высокая: доля прижата к одному значению.

Числа ниже — не пример из учебника, а **вывод самой системы**
(`laplace_posterior` из `backend/app/services/forecast.py`):

| Прогонов | Успехов | Оценка | Интервал 5–95 % | Ширина |
|---:|---:|---:|---|---:|
| 3 | 3 | — | **не показывается** | гейт: нужно ≥ 10 прогонов |
| 10 | 9 | 83 % | **64 … 97 %** | 33 пункта |
| 50 | 45 | 88 % | **80 … 95 %** | 15 пунктов |
| 200 | 180 | 90 % | **86 … 93 %** | 7 пунктов |

Первая строка показательна: при трёх прогонах система **отказывается давать оценку**
и сообщает, сколько наблюдений ещё нужно. Это тот же принцип, что исход «нет данных»
у регламента, — отказ вместо догадки.

**Как читать интервал.** Он говорит не о разбросе результатов, а о **нашем незнании**: чем
меньше прогонов, тем шире вилка. Сузить её можно только наблюдениями.

#### Гейт: оценка не показывается, пока прогонов мало

В РАГРАФ вероятностный уровень **включается при десяти и более наблюдениях**. До того
показывать нечего: интервал шире полусотни пунктов ничего не сообщает читателю, но выглядит
как знание. Лучше честное «данных мало», чем осторожное число, которое прочтут как точное.

---

### XXII.4. Почему правило Лапласа, а не эмпирический байес

**Что такое априорное распределение.** Это то, что мы думаем о доле **до** наблюдений.
У правила Лапласа априор **жёстко задан равномерным** — записывается `Beta(1, 1)` и означает
буквально «любая доля от 0 до 1 равноправна, мы не знаем ничего». Добавление по единице
к успехам и провалам — это он и есть.

**Что такое эмпирический байес.** Априор не задаётся, а **оценивается по когорте** похожих
объектов. Для нас когорта — весь корпус регламентов: если у шестидесяти регламентов доля
успеха обычно около 0,85, то новый регламент разумнее подтягивать к 0,85, а не к 0,5.

**Разница на числах.** Регламент с тремя успехами из трёх:

| Метод | Априор | Оценка | Смысл |
|---|---|---:|---|
| Наивный | нет | 100 % | «Безотказен» — неправда |
| **Правило Лапласа** | равномерный `Beta(1,1)` | **80 %** | «Подтягиваем к полному незнанию» |
| Эмпирический байес | оценён по корпусу, например `Beta(8,2)` | **≈ 85 %** | «Подтягиваем к тому, как ведут себя соседи» |

**Почему сегодня Лаплас.** Три довода, и все практические:

1. **Не требует допущений.** Равномерный априор не вносит в оценку ничего, чего мы
   не наблюдали. Это самая скромная позиция из возможных, и её нельзя оспорить.
2. **Не на чем оценивать когорту.** Журнал прогонов начат недавно; чтобы вывести типичное
   поведение регламентов, нужна история по многим правилам за длительный срок.
3. **Разнородность корпуса.** В корпусе есть учебные регламенты, городские и медицинские —
   считать их одной когортой значит подтягивать медицинский скрининг к поведению учебного
   примера. Прежде чем вводить эмпирический байес, надо решить, **что считать когортой**.

**Что даст переход к эмпирическому байесу, когда данные накопятся.** Ровно одно, но ценное:
**точность оценки для регламентов с короткой историей**. Правило Лапласа тянет их к 0,5 —
к «ничего не знаем», хотя кое-что мы знаем: как ведут себя соседние правила той же области.
Для регламентов с длинной историей разницы не будет никакой: на двухстах прогонах оба метода
дают почти одно и то же.

> **Терминологическая осторожность, и она обязательна.** До сентября 2026 метод назывался
> в коде и документах «эмпирическим байесом», и это было **неверно**: априор фиксирован
> и по когорте не оценивается. Название исправлено. В текстах для внешнего читателя
> корректные формулировки: полная — «байесовская оценка доли с равномерным априорным
> распределением», краткая — «правило следования Лапласа». Писать «эмпирический байес»
> **нельзя до тех пор, пока априор не будет оцениваться по корпусу**.

---

### XXII.5. Как эту оценку читать и как её не читать

| Оценка **означает** | Оценка **не означает** |
|---|---|
| Долю прогонов, в которых критерий решённости был выполнен | Вероятность происшествия на объекте |
| Свойство **записи нормы**: насколько часто правило срабатывает результативно | Свойство объекта: насколько он исправен |
| Накопленный опыт применения правила | Прогноз на конкретный следующий прогон |
| Основание пересмотреть порог, если доля низка | Основание считать вердикт ошибочным |

**Практический смысл в одной фразе.** Низкая доля результативных прогонов — сигнал
не о неисправности объекта, а о том, что **порог, вероятно, выставлен неудачно**: правило
срабатывает там, где реагировать не нужно, либо предписывает действие, которое не решает
задачу. Это инструмент методолога, а не диспетчера.


---

> *Документ создан 2026-05-21. Последнее обновление: 2026-09-25 (Часть XXII — методы ИИ в терминах Центра: два слоя, доверенный ИИ, предъявимость; и разбор вероятностной оценки результативности по шагам — правило Лапласа против эмпирического байеса).*
