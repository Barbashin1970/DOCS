...
**Как считается вероятностный уровень: правило Лапласа.**

Оценивать долю успеха делением «успехи ÷ прогоны» нельзя: три успеха из трёх дадут 100 %, чего быть не может. Применяется **правило следования Лапласа**: к успехам и провалам добавляется по единице, и оценка равна

```
(1 + успехи) / (2 + прогоны)
```

Отсюда двадцать успехов из двадцати дают не 100 %, а около 95 %: уверенность растёт с числом наблюдений, а не объявляется сразу. Формально это апостериорное среднее бета-распределения при равномерном априорном `Beta(1, 1)`; интервал неопределённости — 5-й и 95-й процентили того же распределения, то есть 90-процентный доверительный интервал.

**Чем это отличается от эмпирического байеса.** До сентября 2026 метод в коде и документации назывался эмпирическим байесом, и это было неверно. В эмпирическом байесе априорное распределение **оценивается по когорте** похожих объектов, а здесь оно жёстко задано равномерным. Разница практическая: при эмпирическом байесе регламент с тремя прогонами подтягивался бы к типичному поведению соседних регламентов, а по правилу Лапласа — к 0,5, то есть к «ничего не знаем». Переход к оценке априора по корпусу регламентов числится в планах ([BACKLOG.md](BACKLOG.md)); он уточнил бы прогноз именно для регламентов с короткой историей. До тех пор корректное название метода — правило Лапласа.
...
> *Документ создан 2026-05-21. Последнее обновление: 2026-06-14 (Часть XX — финансовый скоринг расширен вторым банком: РФ Сбер + Мексика Plata, словарь параметров и нормативная база двух юрисдикций).*
