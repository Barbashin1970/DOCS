...
## 12. Что здесь можно защитить (и что — нет)

Метод `d0sl` защищён сторонним правообладателем (Сибирская школа). Flow DSL — **наш** метод,
и его стоит защитить, чтобы лицензировать заказчикам и встраивать в Jira/PMS за плату.
Коротко и честно (подробно — в [`FLOW-DSL-IP-STRATEGY.md`](FLOW-DSL-IP-STRATEGY.md)):

**Что защищаем и чем:**

| Актив | Механизм РФ | Сила |
|---|---|---|
| Код конструктора «Поток» + `flow_executor` + `turtle_bridge` | свидетельство о госрегистрации программы для ЭВМ | сильный (но защищает **код**, не идею) |
| **Способ** исполнения графа: Kleene-обработка «нет данных» + SHACL-ограничение как runtime-gate + AST без `eval` | патент на изобретение (способ) — **только с техническим результатом** | средний / спекулятивный |
| Эвристики линтера, реестры действий/единиц, непубличные части `turtle_bridge` | ноу-хау (режим коммерческой тайны) | средний |
| Библиотека готовых регламентов + онтология | свидетельство на БД (смежное право) | средний |
| Название «РАГРАФ» / «Поток» | товарный знак (кл. 9, 42 МКТУ) | сильный |
| Право заказчиков использовать/встраивать | лицензионный договор / EULA / OEM-сублицензия | сильный (даёт деньги) |

**Самое честно-защитимое (не «весь тандем JSON↔TTL»):** трёхзначная логика Клини, проведённая
сквозь исполняемый граф (offline-датчик → отдельная ветка «уведомить техотдел», а не тихое «всё в норме»),
+ одно и то же SHACL-ограничение, импортированное из формы регламента и применяемое как runtime-gate,
+ безопасное вычисление формул без `eval`. Это узко, но реально и подтверждено кодом.

**Чего заявлять НЕЛЬЗЯ (важно для честности перед экспертизой/инвестором):**
- ❌ «синхронный round-trip всего исполняемого графа в RDF/TTL» — **граф в TTL не сериализуется**
  (`sigma_export.py`: flow.json — ragraf-only). В RDF round-trip'ятся только метаданные регламента.
- ❌ «новый визуальный конструктор» — палитра узлов сама себя документирует как кальку Node-RED (prior art).
- ❌ «гарантированный патент на ПО» — в РФ ПО как таковое патентом не охраняется (п.5 ст.1350 ГК).

**🚨 Два блокирующих факта (требуют решения ДО подачи заявок):**
1. **Репозиторий, по всем признакам, уже публичный** (`github.com/Barbashin1970/RAGRAF`, первый коммит
   13.05.2026; URL фигурирует в презентации и README). Публикация исходников = **раскрытие сути способа**:
   в РФ работает только узкий 6-месячный льготный период (≈ до 13.11.2026), за рубежом новизна уже утрачена.
2. **Правообладатель не определён.** Если код создан в рамках НИР/служебного задания НГУ — исключительное
   право может принадлежать НГУ (ст. 1295 ГК). Лицензировать чужое нельзя.

Полный разбор активов, черновики патентных формул, модели лицензирования, стратегия встраивания в Jira/PMS
и пошаговый план — в [`FLOW-DSL-IP-STRATEGY.md`](FLOW-DSL-IP-STRATEGY.md).
