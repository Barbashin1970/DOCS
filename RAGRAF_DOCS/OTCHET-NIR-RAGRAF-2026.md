...
> [`STRATEGY-POSITIONING.md`](STRATEGY-POSITIONING.md) — позиционирование.
...
|---|---|---|---|
| Разработчик и руководитель НИОКТР | Барбашин О.В. (banksnab@gmail.com) | _____________ | 2026-05-28 |
| Индустриальный партнёр (соисполнитель) | ООО «Группа МЕТА» — Платформа АРХИ | _____________ | 2026-05-28 |
...
| Заказчик стендового развёртывания | ООО «Группа МЕТА» / ЦИИ НГУ (Платформа АРХИ) | Раздел 5 |
...
**Rule DSL Flow (Flow DSL)** — визуальный декларативный язык РАГРАФ для описания регламентов. Семь типов узлов: `input`, `threshold`, `compare`, `formula`, `switch`, `output`, `sensor` (узел `shacl_constraint` убран 2026-06-09 — это контракт ДАННЫХ, а не логика потока). Хранится в `flow.json`. Самостоятельное оформление языка как результата НИР (РИД / кандидат на стандарт) — [RESULT-FLOW-DSL.md](RESULT-FLOW-DSL.md).
...
- [5. Уровень новизны и практическая реализация на стенде](#5-уровень-новизны-и-практическая-реализация-на-стенде)
  ...
  - [5.5. LEYKA как ЦДА — характеристика, стек и обоснование выбора](#55-leyka-как-цда--характеристика-стек-и-обоснование-выбора)
  - [5.6. Внешние интеграции стенда: Plan-R (макет API) и КАППА (экспертная верификация датасетов)](#56-внешние-интеграции-стенда-plan-r-макет-api-и-каппа-экспертная-верификация-датасетов)
...
**Отчёт:** 1 книга, ~78 страниц, 5 рисунков (Mermaid-диаграмм), 21 таблица, 71 источник, 3 приложения.

**Ключевые слова:** ЦИФРОВОЙ ДВОЙНИК СУБЪЕКТА УПРАВЛЕНИЯ, ЗАДАЧНЫЙ ПОДХОД, ТЕОРИЯ ФУНКЦИОНАЛЬНЫХ СИСТЕМ, ОНТОЛОГИЧЕСКАЯ МОДЕЛЬ ПАЛЬЧУНОВА, ЗАМКНУТЫЙ КОНТУР, FORMAL-PROOF XAI, SHACL, DUCKDB, FASTAPI, W3C TURTLE, PROV-O.

**Документ-основание:** инициативная разработка с использованием открытых публикаций Сибирской математической школы (С.С. Гончаров, Д.И. Свириденко, Е.Е. Витяев) и НГУ (Д.Е. Пальчунов). Связь со стендовой реализацией для ООО «Группа МЕТА» (индустриальный партнёр, объект «Платформа АРХИ») закреплена через ТЗ АРХИ (11710-11710-1-ЛУ, 2024) и [`PROGRESS-ARHI-2026-05-27.md`](PROGRESS-ARHI-2026-05-27.md).

**Цель НИР.** Разработать и довести до уровня опытной эксплуатации (УГТ-7) программную платформу РАГРАФ, реализующую замкнутый контур «датчик — регламент — задача — человек» на основе задачного подхода и формально-проверяемой логики, и продемонстрировать её применимость в задачах управления городскими инфраструктурами на примере домена «Строительство и инфраструктура» (Платформа АРХИ). Принципиальное позиционирование, раскрытое в специальном подразделе Введения: **Σ Сигма — фреймворк** (набор инструментов для разработчиков); **Платформа АРХИ — прикладная платформа** (по ТЗ ориентирована на функционирование в составе Σ Сигма); **РАГРАФ — рабочий прототип фреймворка Σ Сигма** на упрощённом стеке, демонстрирующий **пригодность набора инструментов для сборки прикладных систем класса АРХИ** (78% функционального и 80% архитектурного покрытия требований ТЗ АРХИ, см. §5.8).
...
**Раздел 5** содержит описание **научной и технической новизны** платформы, **уровня готовности технологии (УГТ-7)** и **практической реализации на стенде**: 4 строительных цифровых двойника регламентов (ПДЭ/просрочка/материалы/портфельный риск), отдельную характеристику LEYKA как цифрового диалогового ассистента (ЦДА — система управления проектами со встроенной мессенджер-Лентой в стиле Telegram, см. §5.5), две внешние интеграции стенда — Plan-R и КАППА (§5.6); в части Plan-R помимо макет-сервера с дублированием публичного API v602 описан **начатый прямой диалог с командой Plan-R**, **согласованная модель интеграции** «реестр нарушений + связь с задачами WBS через кастомные атрибуты EPS/WBS», **карта 4 логических контуров API** (EPS / WBS / Versions / Spaces, 25 эндпоинтов) и **дорожная карта боевой интеграции** на 3 рабочие недели; в части КАППА — фреймворк экспертной верификации триплет-датасетов с возвратом подписи эксперта-человека; раздел также содержит двустороннюю интеграцию РАГРАФ ↔ LEYKA, 4 демонстрационных сценария, проверенных на продакшен-развёртывании (https://ragraf.up.railway.app), матрица покрытия 18 из 23 требований §4.2 ТЗ АРХИ (≈78%), архитектурное совпадение с §4.3 ТЗ АРХИ на ≈80%, метрика точности принятия решений 90–93%, **образовательный модуль РАГРАФ как самостоятельный канал тиражирования** (программа ДПО на 72 ак. ч., тематический план первого потока, KPI образовательного модуля — см. §5.11).

**Раздел 6** содержит общее заключение по результатам НИОКТР, **анализ архитектурных ограничений текущей реализации** (модель «один писатель» в DuckDB) и **дифференцированные пути их снятия в боевых внедрениях по восьми секторам продуктовой ниши** (§6.6), а также план дальнейших работ: завершение Phase C.2 (switch-нода real-eval) и Phase D.2 (auto-create LEYKA-задач с дедупликацией эпизодов) для достижения целевой точности 95%+, **переход с макета Plan-R на боевой ETL-адаптер по согласованной модели** «реестр нарушений + атрибуты EPS/WBS» (3 этапа, ≈ 3 рабочие недели после получения API-ключа от команды Plan-R), типизация цифрового двойника договора (ЦДД), а также **дорожную карту расширения Flow DSL в гибридный контур** (§6.12) — пять конкретных инженерных шагов на ≈ 3.5 рабочие недели, формализующих наблюдаемые в коде проявления правого полушария (multi-hypothesis в демо-экране «Платформа АРХИ», `confidence` в схемах датчиков) и переводящих их в сквозной pipeline `confidence → physics cross-check → multi-hypothesis → двухпанельная карточка двойника → версионные контракты слоёв`, с подтверждённым ростом покрытия требований ТЗ Σ СИГМА с ≈ 87% до ≈ 92%.
...
**Документ-основание:** инициативная разработка одного автора (Барбашин О.В.) с опорой на открытые публикации Сибирской математической школы. Связь с индустриальной кооперацией закреплена через стендовое развёртывание для ООО «Группа МЕТА» и ЦИИ НГУ (Платформа АРХИ, ТЗ 11710-11710-1-ЛУ от 2024).

**Техническое задание:** [`docs/TZ_RAGRAF.md`](TZ_RAGRAF.md), версия 2.0 от 2026-05-23, идентификатор РГРФ-02-01-01-1/1-ТЗ.
...
Это **принципиальное архитектурное решение** оказывает влияние на всё последующее изложение: на выбор технологического стека (раздел 2.3, 2.4), на математическую базу (раздел 3 — задачный подход и теория функциональных систем, а не машинное обучение), на состав фикстур-регламентов (приложение А), на метрику качества (раздел 5.7 — точность принятия решений рассчитывается для **декларативных регламентов**, а не для LLM-ответов), на нишевое позиционирование (раздел 5.8 — РАГРАФ есть инструмент в категориях «семантическое моделирование» и «формальная верификация», а не в категории «генеративный ИИ»).
...
Результатом решения задачи является программный продукт, описанный в настоящем отчёте: платформа РАГРАФ (репозиторий https://github.com/Barbashin1970/RAGRAF), реализующая ЦД СУ для четырёх приоритетных доменов (стройплощадка / ЖКХ / ИТ-команда / госуправление) на стенде индустриального партнёра ООО «Группа МЕТА» и ЦИИ НГУ.
...
>
> — ТЗ Платформы АРХИ 11710-11710-1-ЛУ, 2024.
...
На рынке РФ и СНГ 2026 года **отсутствует целостный инструмент**, который одновременно работает над любыми датчиками (IoT, видеоаналитика, оптоволокно), любыми таск-трекерами (LEYKA, Jira, YouTrack) и любыми мессенджерами (Telegram, ВК, корпоративные каналы) и при этом обеспечивает **формальную проверяемость** логики реагирования (не «чёрный ящик» нейросети, а декларативный DSL с трассировкой каждого решения до пункта нормативного акта).
...
2. Замкнутый контур принятия решений, обеспечивающий точность ≥ 90% по агрегату решений (см. раздел 5.7).
3. Стендовое развёртывание с 4 строительными ЦДР, двусторонней интеграцией с LEYKA как ЦДА, демонстрационными сценариями для индустриального партнёра.
4. Архитектурное совпадение с требованиями §4.3 ТЗ АРХИ на уровне ≥ 80%, функциональное покрытие §4.2 на уровне ≥ 75%.
...
6. **Самостоятельный результат интеллектуальной деятельности (РИД) — визуальный язык цифровых регламентов Flow DSL.** Сложился в ходе НИР (гипотеза графического описания регламента → устойчивый синтаксис: 5 узлов ядра + 2 «сахарных» для учеников + узел критерия решённости; родословная Σ-программирование → d0sl → Flow DSL). Выходит за рамки ПО: независим от конкретной реализации, кандидат на **стандартизацию** (открытая спецификация) и передачу индустриальному партнёру (напр. СБЕР). Оформление результата — [RESULT-FLOW-DSL.md](RESULT-FLOW-DSL.md).
...
|---|---|
| **1. Предметная область** | Онтология домена в OWL/Turtle/SHACL. Хранится как `data.ttl` + `shapes.ttl` + REGISTRY в `backend/data/fixtures/`. Содержит классы объектов (датчик, регламент, исполнитель, задача), их сигнатуры и аксиомы. |
...
|---|---|---|
| Ф-1 | Приём событий из произвольных внешних систем через REST/HTTP-опрос | [`etl/puller.py`](../backend/app/services/etl/puller.py) — Open-Meteo, Yandex.Traffic, LEYKA, NSK OpenData, RAGRAF Clock |
...
| Ф-6 | Асинхронная проверка критерия (через датчик-последователь или обратный вызов LEYKA) | [`acceptance_resolver.resolve_pending`](../backend/app/services/etl/acceptance_resolver.py) — TTL `max(7д, sla×2, follow×5)` |
| Ф-7 | Создание задач в таск-трекере при срабатывании регламента | [`leyka_task_creator`](../backend/app/services/leyka_task_creator.py) + диалог пользовательского интерфейса `LinkToLeykaDialog` |
| Ф-8 | Подсказки методологу при дрейфе `success_rate` | [`metric_analyzer.compute_hints`](../backend/app/services/metric_analyzer.py) — 7д против 14д, перекосы, тайм-ауты |
...
| Ф-12 | Двусторонняя интеграция с таск-трекером (output→задача, sensor←смена статуса) | `LinkToLeykaDialog` + `LinkSensorToLeykaDialog` + [`leyka_task_poller.py`](../backend/app/services/leyka_task_poller.py) — 30 с поллер |
...
| НФ-6 | Соответствие ГОСТ Р 59276-2020 «Системы ИИ. Способы обеспечения доверия» | Реализовано (см. раздел 3.4) |
...
**ЭТАП 6 — Тестирование и валидация.** SHACL-валидация критерия, тестовое исполнение регламента через `POST /api/regulations/{id}/execute`, проверка трассы исполнения и `regulation_metrics`. **Инструмент:** `ExecuteScreen` + `CriterionValidationBanner` в РАГРАФ.
...
| **Городские платформы** (Σ Сигма, Минстрой «Умный город», Ростелеком ИАЦ) | масштаб города, единая база знаний | лёгкий стартовый вход для команды на 10 регламентов, развёртывание на ноутбуке |
...
3. **PROV-O для аудита.** Стандарт PROV-O (Provenance Ontology) даёт готовые конструкции для аудита: `prov:wasDerivedFrom`, `prov:wasGeneratedBy`, `prov:Agent`. Это закрывает требование §4.2.9 ТЗ АРХИ — объяснимость по построению, нужная для прохождения регуляторных проверок.

4. **SHACL для декларативной валидации критерия.** SHACL-shapes конфигурации регламента (например, `:AcceptanceCriterionShape` с условными правилами per kind через `sh:or`) проверяются на сохранении регламента — методолог видит mis-config'и до боевого исполнения, что повышает точность принятия решений на 5–7 п.п. (см. раздел 5.7).
...
|---|---|---|---|
| Python | 3.13 | PSF | Современная стабильная версия с typing improvements, asyncio TaskGroup, performance gains |
...
| httpx | 0.27 | BSD | Async HTTP-client для LEYKA, Open-Meteo, Yandex.Traffic, Plan-R |
| openai (SDK) | 2.36 | Apache 2.0 | OpenAI-compatible клиент для Ollama / Cerebras / Groq / OpenRouter / OpenAI / mock — одна переменная LLM_PROVIDER переключает |
...
| Zustand | 4 | MIT | Локальное состояние без шаблонного кода в стиле Redux |
| React Flow | 11 | MIT | Готовый редактор графов для Rule DSL, поддерживает custom-узлы и edge condition |
| @dnd-kit | 6 | MIT | Drag-and-drop для перетаскивания узлов между колонками |
...
**Деплой:** Railway (текущий прод-стенд) + Fly.io (резервный) + Vercel (только лендинг + landing-демо). Все три платформы поддерживаются скриптами CI/CD в репозитории.

### 2.4. Соотношение технологических стеков РАГРАФ и Σ Сигма

РАГРАФ и Σ Сигма — родственные продукты ЦИИ НГУ, ориентированные на разный масштаб развёртывания и разный класс пользователя. Технологические стеки обоих фреймворков частично пересекаются (общий W3C-семантический контур, общий язык Python, общий веб-фреймворк FastAPI), но различаются в выборе слоя хранения и в способе доставки до конечного пользователя.
...
|---|---|
| Python 3.13 | То же: серверная часть, ETL, исполнение регламентов, интеграции |
...
| События | Pull-based (ETL-puller с интервалом N минут) | Push-based, через очереди (Kafka/RabbitMQ — план) |
...
| 5. Исполнение | `leyka_task_creator.ensure_task` (push в LEYKA) + автокомментарии в чат задачи |
| 6. Обратная афферентация | `acceptance_resolver.resolve_pending` + `metric_analyzer.compute_hints` → подсказки методологу |
...
2. Промаркированы фикстуры по 5 типам тестовых наборов §9 ГОСТ Р 59898 в [`backend/data/fixtures/INDEX.md`](../backend/data/fixtures/INDEX.md) (базовый_демонстрационный = домен `tutorial`, дополнительный = боевые домены, полный = весь корпус 31 регламент, обучающий = `knowledge_seed/`, тестовый = `backend/tests/`).
...
([`project_palchunov_dynamic_picture`](../backend/data/demo_documents/GLOSSARY.md))
— к гибридным.
...
Полное архитектурное описание содержится в [`ARCHITECTURE.md`](ARCHITECTURE.md) (848 строк). В данном разделе приводится сводное изложение для целей НИР-отчёта.
...
flowchart TD
    ...
    ANA["metric_analyzer.compute_hints<br/>дрейф 7d vs 14d, перекосы<br/>(Phase E)"]
...
| **TTL** (`data.ttl` + `shapes.ttl`) | upstream / Σ Сигма | Outbound interchange | Сериализация для Σ Сигма / Apache Jena, parsable стандартными RDF-инструментами |
...
1. **Данные от внешних Оракулов:**
   ...
   - Таск-трекеры через REST API (LEYKA, Jira, YouTrack);
   - Бизнес-системы (Plan-R PMS, 1C, ЭДО);
...
3. **Неструктурированные данные:**
   - PDF-документы регламентов (через Knowledge Ingest Studio);
...
1. **Результаты анализа и прогнозов:**
   ...
   - Подсказки методологу (`metric_analyzer.compute_hints`).

2. **События и команды:**
   - Создание задач в LEYKA через `leyka_task_creator.ensure_task`;
   - Автокомментарии в чат задачи при срабатывании регламента (`leyka_pings[]`);
...
- Платформа слушает только на `127.0.0.1` (loopback) — снаружи компьютера недоступна;
...
2. **Инфраструктура.** Из требования «получение событий, валидация, исполнение, порождение событий, выдача рекомендаций» прямо вытекает потоковая архитектура Source → Processing → Knowledge → Presentation (§4.1). Из требования «постоянной части ЦД» вытекает выделение Storage Layer как отдельного слоя с гарантиями durability (DuckDB WAL, ADR-001, ADR-020). Из требования «синхронного получения информации» — REST API контракт (FastAPI, 70+ эндпоинтов). Итог: **операционная платформа** РАГРАФ = пять слоёв из §4.1, реализующих жизненный цикл события от датчика до методолога.
...
|---|---|---|---|
| **Получение событий** | Источники с периодическим опросом (ETL puller, LEYKA poller, Clock); источники с инициативой передачи (ручной запуск через `/execute`) | [`backend/app/services/etl_pull.py`](../backend/app/services/etl_pull.py), [`backend/app/services/leyka_task_poller.py`](../backend/app/services/leyka_task_poller.py), `POST /api/regulations/{id}/execute` | Слой источников / Слой обработки |
| **Валидация события** | Мягкая проверка по `sensor_field_schemas` (фаза F.1) — предупреждение при неизвестном поле, не блокирует конвейер (унаследованные источники меняют формат) | [`backend/app/services/etl_validate.py`](../backend/app/services/etl_validate.py), [`backend/app/services/sensor_field_schemas.py`](../backend/app/services/sensor_field_schemas.py) | Слой обработки |
| **Обработка (исполнение спецификаций)** | Интерпретатор декларативного DSL: `compare` / `switch` / `formula` с настоящим вычислением условий (фазы C.1+C.2); синтетический интерпретатор как резервный путь (ADR-007) | [`backend/app/services/flow_executor.py`](../backend/app/services/flow_executor.py), [`backend/app/services/applier.py`](../backend/app/services/applier.py) | Слой обработки |
| **Порождение производных событий** | `fired_outputs` из исполнения становятся новыми событиями для резолвера приёмки и моста LEYKA; срабатывание sensor-узла, привязанного к LEYKA-задаче, повторно входит в конвейер через поллер | [`backend/app/services/acceptance_resolver.py`](../backend/app/services/acceptance_resolver.py), `leyka_task_poller._maybe_fire_regulation` | Слой обработки |
...
| **Синхронное получение информации** (запрос к ЦДСУ) | REST API на FastAPI, OpenAPI-описание, 70+ точек доступа: `/api/regulations`, `/api/templates`, `/api/sensor-schemas`, `/api/flow/*`, `/api/metrics/*`, `/api/audit/*` | [`backend/app/main.py`](../backend/app/main.py), [`backend/app/api/`](../backend/app/api/) | Слой представления |
| **Модели управления доступом (безопасность)** | `ADMIN_PASS` + HMAC-кука, `Depends(require_admin)` на разрушающих точках доступа, `hmac.compare_digest` для сравнения за константное время, привязка к loopback-адресу (ADR-019) | [`backend/app/security.py`](../backend/app/security.py) | Слой представления / Слой хранения |
| **Работа со знаниями** | Загрузка Turtle-онтологий + SHACL-форм + Knowledge Studio (CSV→RDF, SPARQL Workbench, Cytoscape-визуализация) + retrieval-слой Студии аналитика (bge-m3 эмбеддинги, опциональная LLM-генерация); `KnowledgeGraph` создаётся на каждый регламент | [`backend/app/services/turtle_bridge.py`](../backend/app/services/turtle_bridge.py), [`backend/app/services/knowledge_graph.py`](../backend/app/services/knowledge_graph.py), [`backend/app/services/sparql_runner.py`](../backend/app/services/sparql_runner.py) | Слой знаний |
...
| **Трассировка решений (объяснимость)** | PROV-O запись для каждого вердикта: источник, версия регламента, цепочка сработавших узлов; история редакций через `regulation_versions` | [`backend/app/services/turtle_bridge.py`](../backend/app/services/turtle_bridge.py) (PROV-O), [`backend/app/services/regulation_store.py`](../backend/app/services/regulation_store.py) (versions) | Слой знаний |
| **Замыкание контура (приёмка + подкрепление)** | Трёхзначное `criterion_passed` (TRUE/FALSE/NULL=ожидание), TTL на критерий `max(7д, sla×2, follow×5)` (ADR-008), последующая проверка через датчик или обратный вызов LEYKA | [`backend/app/services/acceptance_resolver.py`](../backend/app/services/acceptance_resolver.py), [`backend/app/services/metric_analyzer.py`](../backend/app/services/metric_analyzer.py) | Слой обработки |
| **Диагностика и подсказки методологу** | `metric_analyzer.compute_hints` — дрейф 7д против 14д, перекосы по доменам; «полоса здоровья» в пользовательском интерфейсе; экран журнала инцидентов | [`backend/app/services/metric_analyzer.py`](../backend/app/services/metric_analyzer.py), [`frontend/src/components/HealthBanner.tsx`](../frontend/src/components/HealthBanner.tsx) | Слой обработки / Слой представления |
...
Раздел содержит описание научной и технической новизны платформы, уровня готовности технологии (УГТ-7) и **практической реализации на продакшен-стенде** для индустриального партнёра ООО «Группа МЕТА» в рамках проекта Платформа АРХИ ЦИИ НГУ.
...
**Стенд РАГРАФ × Платформа АРХИ × Группа МЕТА** развёрнут на проде в связке РАГРАФ ↔ LEYKA для демонстрации индустриальному партнёру (ООО «Группа МЕТА») и ЦИИ НГУ.

| Компонент стенда | URL | Назначение |
|---|---|---|
| РАГРАФ web (фронт+бэк) | https://ragraf.up.railway.app | Flow Editor, Regulation Editor, Studio аналитика, `/demo-arhi` |
| РАГРАФ API (OpenAPI) | https://ragraf.up.railway.app/docs | Swagger для интегратора |
| LEYKA фронт | https://leyka-nsu.vercel.app | ЦДА: задачи, чат, доска, статусы |
| LEYKA backend API | https://leyka-production.up.railway.app/api/v1 | Бэк ЦДА (cookie-auth) |
...
| Демо-экран «Один день АРХИ» | https://ragraf.up.railway.app/demo-arhi | Лента из 12 событий за смоделированный рабочий день + капабилити-карта Plan-R |
...
| Возможность отдать заказчику как один инсталлятор | Заказчик (аналитик-методолог в ООО «Группа МЕТА») запускает РАГРАФ на своём рабочем ноутбуке без обращения в свою службу ИТ; срок «от первой встречи до первого работающего регламента» сокращается с недель до часов |
...
**Состав 4 строительных ЦДР** (живут как фикстуры в `backend/data/fixtures/construction-*`):
...
4. **`construction-portfolio-risk`** — портфельный риск-скоринг. Триггер: мульти-сенсор по всем объектам портфеля. Логика: `max(overdue_days) × N(critical-этапов) × стоимость договора → risk-score`. SHACL чип «risk-score > порог инвестора». Связан с регламентом №2 через `sourceKind='regulation'` — композиция ЦДР.

### 5.5. LEYKA как ЦДА — характеристика, стек и обоснование выбора
...
| Слой | РАГРАФ | LEYKA |
|---|---|---|
| Язык бэкенда | Python 3.13 | Python 3.12 |
...
3. **Работа без VPN на российской инфраструктуре.** В условиях 2025–2026 годов, когда классические зарубежные таск-трекеры (Jira Cloud, Slack-интеграции) требуют VPN для государственных учреждений, LEYKA работает на российских доменах без ограничений доступа. Это важно для тиражирования стенда на партнёра ООО «Группа МЕТА» и на ЦИИ НГУ.
...
### 5.6. Внешние интеграции стенда: Plan-R (макет API) и КАППА (экспертная верификация датасетов)
...
| Капабилити-карта | Документация «что умеем сейчас / что в backlog / ROI» | [demo-arhi экран](https://ragraf.up.railway.app/demo-arhi); [`PLAN-R-MOCK.md`](PLAN-R-MOCK.md) |
...
**Сценарий 4: «Один день на Платформе АРХИ» (демо-экран для инвестора).** URL: https://ragraf.up.railway.app/demo-arhi (рендерится `DemoArhiScreen.tsx`). Endpoint: `GET /api/demo/arhi-feed` отдаёт ленту из 12 событий за смоделированный рабочий день + капабилити-карту Plan-R (что умеем, что в backlog, ROI) + **карту возможностей публичного API Plan-R v602** с разбивкой на 4 логических контура EPS/WBS/Versions/Spaces и явным указанием согласованной модели «реестр нарушений + связь с задачами WBS». Используется в питче инвесторам: «вот один день, который сейчас прошёл бы без визибилити для руководителя — а с РАГРАФ он видит каждый риск в момент его возникновения». Не пишет в боевые сторы, идемпотентно.
...
|---|---|---|---|---|---|
| 1 | **Строительство (генподряд, девелопмент)** | СИЗ, просрочка этапов, входной контроль материалов, портфельный риск | ИИ-прораб + риск-менеджер портфеля | Быстрый (недели) | Стенд + 4 ЦДР, индустриальный партнёр ООО «Группа МЕТА» |
...
1. **Время от первой встречи с заказчиком до первого работающего регламента** (нацело — часы / дни / недели). Измеряется по фактическим внедрениям. На стенде РАГРАФ + Группа МЕТА — 5 человеко-дней.
...
Накопление этих метрик ведётся в `regulation_metrics` и в файлах [`PROGRESS-ARHI-2026-05-27.md`](PROGRESS-ARHI-2026-05-27.md) и [`STRATEGY-POSITIONING.md`](STRATEGY-POSITIONING.md). По итогам этапа 2 НИОКТР планируется отдельный аналитический отчёт «Продуктовая ниша РАГРАФ — карта секторов и темпы принятия» как самостоятельный результат интеллектуальной деятельности.
...
3. **Архитектурный подход «ЦД как ЦД субъекта управления»** (а не объекта) — методологически закреплён, проверен на 4 строительных ЦДР, отделяет РАГРАФ от классического понимания цифрового двойника как BIM-модели физического объекта.
...
**Результат.** Те же потоки данных от внешнего события до подсказки методологу реализуемы **без триплет-стора**. Эмпирическая граница комфорта DuckDB на одном инстансе — **до 1 000 регламентов** (типовой профиль: один аналитик-методолог управляет 50–300 регламентами; крупный центр или подразделение — 300–1 000); за этой границей открываются ограничения «один писатель», которые рассматриваются отдельно в разделе 6.6. Производительность SQL-запросов по `regulations.duckdb` (≈ 5–15 мс на типовой запрос) **превосходит** SPARQL-запросы на Apache Jena с теми же данными (≈ 50–200 мс).
...
2. На стенде реализованы **4 строительных ЦДР** для индустриального партнёра ООО «Группа МЕТА»: контроль СИЗ, просрочка этапа, входной контроль материалов, портфельный риск.
...
2. Боевой адаптер к Plan-R API v602 по согласованной с командой Plan-R модели «реестр нарушений + атрибуты EPS/WBS» (3 рабочие недели, ≈ 15 человеко-дней, после получения API-ключа — детальная дорожная карта в §5.6.1).
...
5. Telegram-бот как альтернативный канал ЦДА для непользователей LEYKA (3 человеко-дня).
...
**Все поставленные на этап 1 задачи выполнены в полном объёме. Полученные результаты соответствуют запланированным и существенно превзошли ожидания по части экспериментальных находок (см. раздел 6.2) и продуктовой ниши (см. разделы 5.10 и 6.7). Целесообразно продолжить НИОКТР согласно утверждённому плану.**
...
|---|---|---|
| Multi-hypothesis с консенсусом | Демо-экран «Один день на Платформе АРХИ»: на каждый warn-инцидент ИИ-помощник предлагает **2–3 варианта решения** с явной маркировкой одного как «РЕКОМЕНДУЕТСЯ» | [`frontend/src/components/demo-arhi/DemoArhiScreen.tsx`](../frontend/src/components/demo-arhi/DemoArhiScreen.tsx) + [`backend/data/fixtures/demo-arhi-day.json`](../backend/data/fixtures/demo-arhi-day.json) |
...
**Принципиальное наблюдение.** Демо-экран «Платформа АРХИ» содержит **multi-hypothesis в зачаточной форме**: три варианта решения для каждого warn-инцидента (например, для расхождения накладной — «физическая сверка + фото», «претензия поставщику», «приёмка с актом») с одним вариантом как рекомендуемым. В текущей реализации эти три варианта **жёстко зашиты в фикстуре** `demo-arhi-day.json`. Шаг к гибриду — научить Flow DSL **генерировать** эти варианты на лету по контексту инцидента и приходящим параметрам. Это и есть содержание шага 3 ниже.
...
38. [`docs/STRATEGY-POSITIONING.md`](STRATEGY-POSITIONING.md) — Стратегический отчёт по позиционированию РАГРАФ.
...
43. [`docs/PLAN-R-MOCK.md`](PLAN-R-MOCK.md) — Plan-R Public API v602 mock-сервер.
...
53. [`docs/ragraf_economics_marketing_report.md`](ragraf_economics_marketing_report.md) — Экономический отчёт (TCO/ROI по секторам).
...
71. Plan-R Public API v602 — система оперативного управления строительным проектом (PMS); подключена к стенду РАГРАФ через макет-сервер с полным дублированием публичного API. Макет-реализация: [`backend/app/api/_planr_mock.py`](../backend/app/api/_planr_mock.py); сопровождающая документация: [`docs/PLAN-R-MOCK.md`](PLAN-R-MOCK.md). См. раздел 5.6.1.

---

## Приложение А. Состав фикстур регламентов

Полный список цифровых регламентов (ЦДР), реализованных в платформе РАГРАФ на момент составления отчёта (2026-05-28). Каждый регламент представлен тремя файлами: `data.ttl` (параметры), `shapes.ttl` (SHACL-ограничения), `flow.json` (визуальный DSL).

**Домен «Строительство и инфраструктура» (ЦИИ НГУ × Группа МЕТА — основное направление):**
...
| 4 | `construction-portfolio-risk` | Портфельный риск-скоринг | `custom` |
...
| 7 | `weight-tolerance` | Отклонение веса груза от тендера |
...
| 10 | `air-pdk-exceeded` | Превышение ПДК в воздухе → SMS |
...
| № | Source ID | Назначение |
|---|---|---|
| 13–17 | `it-support-*` | 5 регламентов IT-инцидентов |
...
| 19 | `leyka-task-stale-inbox` | Долго лежащие задачи в Inbox |
| 20 | `leyka-bug-triage` | Распределение баг-репортов по приоритету |

**Прочие домены** (социальная сфера, диагностика, безопасность): см. полный реестр в [`backend/data/fixtures/`](../backend/data/fixtures/).

**Итого: 40 регламентов в 6+ доменах**, доступны через `GET /api/regulations`.

---

## Приложение Б. API-эндпоинты платформы

Публичные REST-эндпоинты РАГРАФ (выборка). Полный список — в OpenAPI документации по адресу https://ragraf.up.railway.app/docs.

**Регламенты (CRUD + версионирование):**

| Метод | Эндпоинт | Назначение |
|---|---|---|
| GET | `/api/regulations` | Список всех регламентов с фильтрацией |
...
| GET | `/api/regulations/{id}/history` | История версий регламента |
...
| POST | `/api/etl/sources/{id}/toggle` | Включить/выключить источник |
| POST | `/api/live/tick` | Ручной триггер ETL-тика (для отладки) |
| GET | `/api/live/scheduler-status` | Статус scheduler'а |
...
| Метод | Эндпоинт | Назначение |
|---|---|---|
| GET | `/api/coverage` | Структурный pre-flight (5 видов «дыр») |
| GET | `/api/knowledge/datasets` | Список SEED + user datasets |
| POST | `/api/knowledge/datasets/{id}/sparql` | SPARQL playground |
| POST | `/api/sandbox/chat` | Студия аналитика — чат над корпусом |
| POST | `/api/sandbox/extract` | Извлечь регламент из PDF/DOCX |

**Sigma-integration:**

| Метод | Эндпоинт | Назначение |
|---|---|---|
| GET | `/api/sigma-export/regulation/{id}` | Bundle (data.ttl + shapes.ttl + manifest) |
| POST | `/api/sigma-import/bundle` | Импорт пакета регламентов от Σ Сигма |
| POST | `/api/admin/reseed` | (admin) Пересеять фикстуры |
...
| ЛНК / СУБД | Система управления базами данных |
...
| ЦД | Цифровой двойник |
| ЦДА | **Цифровой Диалоговый Ассистент** — система управления проектами (СУП) со встроенной мессенджер-Лентой, через которую система ведёт диалог с человеком-исполнителем (постановка задачи, статусы, комментарии в стиле Telegram-сообщений). См. также раздел В.2. |
...
**Действенная подсказка (англ. *actionable hint*)** — рекомендация методологу с конкретным предложением правки регламента, формируемая `metric_analyzer.compute_hints`.
...
**Паспорт прикладного модуля** — описание модуля по 7 разделам: контракт API, правила качества, типы событий, контакты. Хранится в таблице `modules`.
...
**Полоса здоровья (англ. *health banner*)** — индикатор состояния источника событий в пользовательском интерфейсе. Реализован в `HealthBanner.tsx`.
...
**Серверная часть (англ. *backend*)** — программный слой, выполняющий обработку запросов и работающий с базой данных. В РАГРАФ — FastAPI-приложение на Python 3.13.
...
**Фикстура** — встроенный пример регламента, поставляемый с дистрибутивом РАГРАФ. Хранится в `backend/data/fixtures/<domain>/`.
...
**СУП — Система управления проектами** — программная система для ведения портфеля проектов с учётом задач, сроков, ответственных, бюджетов и документации. В РАГРАФ-контуре СУП-частью ЦДА является LEYKA с её канбан-доской и проектным деревом; мессенджер-часть ЦДА (Лента в стиле Telegram) встроена в ту же систему, что и СУП. Сочетание «СУП + встроенный мессенджер» — отличительная особенность LEYKA как ЦДА от классических связок «Jira + Slack/Telegram».
...
**KnowledgeGraph** — структура в РАГРАФ, представляющая знания по конкретному регламенту в виде графа.
...
**REGISTRY** — реестр доменных моделей в каждой фикстуре; описывает классы, сигнатуры и аксиомы предметной области.
...
**Telegram-бот** — альтернативный канал ЦДА; запланирован к реализации в этапе 2 НИОКТР.
...
**Дата составления документа:** 2026-05-28.
**Версия документа:** 1.0.
