...
5. Эмпирическая оценка по трём вопросам на корпусе из 59 регламентов и промышленной платформе (раздел 7).
...
Материал оценки — корпус из **59 регламентов в 15 предметных областях**, написанных для реальных задач (теплоснабжение, ЖКХ, экология, дорожный трафик, диспетчерское реагирование, строительство, кампусная инфраструктура, техническая поддержка, кредитный скоринг, медицинский скрининг), и промышленная платформа, собранная на том же ядре.
...
**Таблица 7.1.** Результат анализа корпуса

| Показатель | Значение |
|---|---|
| Регламентов проанализировано | 59 |
| Регламентов без замечаний | 45 (76 %) |
| Регламентов с замечаниями | **14 (24 %)** |
| Замечаний всего | 15 |
| Классов замечаний | 2 |
...
|---|---|---|
| Отсутствие ветви на случай «неизвестно» | **9** | при молчащем источнике данных регламент не выдаст вердикта вовсе: ни нормы, ни тревоги |
...
Существенно, что дефект найден в **девяти регламентах из пятидесяти девяти (15 %)**, написанных предметными специалистами и прошедших ручную проверку. Это не ошибки новичков: пропуск ветви «данных нет» не виден при чтении графа, потому что глазом читается сценарий, в котором данные есть.

Второй класс (недостижимый узел) обнаружим и в двузначной семантике, но требует представления регламента как графа с прослеживаемыми путями, то есть той же проекции.

Все пятнадцать замечаний имеют степень «предупреждение»: регламенты работоспособны, но в перечисленных условиях ведут себя не так, как ожидает автор. Ни одно из них не было обнаружено ни исполнением, ни тестированием — только анализом.
...
Трёхзначная семантика делает это различие выразимым, а статический анализ — обнаружимым. Измерение на корпусе из 59 регламентов, написанных предметными специалистами (раздел 7.2), показало, что ветвь на случай отсутствия данных **не предусмотрена в каждом седьмом**. Это не гипотетический риск: в жёстко записанном виде те же девять правил давали бы ложное «в норме» при потере сигнала, и никто бы об этом не узнал.
