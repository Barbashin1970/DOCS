...
Версия: 2.0 (2026-05-23) — полная переработка под задачный подход и
целостный поток данных.
...
| **Связанные документы** | DATA-FLOW-AUDIT.md (поток данных), STRATEGY-POSITIONING.md (позиционирование), GLOSSARY.md (термины) |
...
- **Редактор:** Knowledge Studio (CSV → RDF), Regulation Editor (формы / слайдеры / Turtle),
  SPARQL playground с 33 пресетами.
- **Объём фикстур:** 13 seed-датасетов (1374 триплета) для Smart-City доменов.
...
| 2  | Принятие решения        | Rule DSL: 8 типов узлов, compare с реальным operator (Phase C.1), switch как маршрутизатор по condition (Phase C.2) |
...
- **F.1.2** Visual Flow Editor (React Flow, 8 типов узлов).
- **F.1.3** SHACL constraint editor + SHACL shape для AcceptanceCriterion.
- **F.1.4** SPARQL playground с 33 пресетами.
...
- Single-secret admin auth через `ADMIN_PASS` + HMAC-cookie.
- `hmac.compare_digest` для constant-time сравнения.
- Cookies httpOnly + secure на https.
